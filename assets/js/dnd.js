...
function onMouseMove(e){
    ...
    if(e.buttons === 1){
        if(imgPos.x <= 0 && imgPos.y <= 0){
            if(imgPos.x > -img.width + canvas.width && imgPos.y > -img.height + canvas.height){
                var dx=e.movementX;
                var dy=e.movementY;
            ...
            }else if(imgPos.x <= -img.width + canvas.width){
                imgPos.x = -img.width + canvas.width+1
            }else if(imgPos.y <= -img.height + canvas.height){
                imgPos.y = -img.height + canvas.height+1
            }
        }else{
            if(imgPos.x > 0){
                imgPos.x = 0
...
function whereAmI(e){
    ...
    let tiles = {
        tileX:Math.floor(x/70) + -imgPos.x/70,
        tileY:Math.floor(y/70) + -imgPos.y/70,
    }
    return tiles
}
...
function noteOnMouseMove(e){
    if(selectionDown){
        ...
        // console.log(movex)
        // console.log(movey)
        // console.log(mouseOriginPosition)
...
function createNote(x,y,w,h){
    ...
    notePos = {
        ...
        endX: Math.ceil(origin.x + x)/tileSize + w/70,
        endY: Math.ceil(origin.y + y)/tileSize + h/70,
    }
...
function tokenCMOnDown(e){
    if(e.buttons === 2){
        if(isOnTokenCM(e)){
            ...
            cMenu.style = `top:${y}px;left:${x+tileSize}px;width:${tileSize*1.5}px;height:${tileSize*2}px`
